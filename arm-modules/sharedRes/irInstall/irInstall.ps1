...
function Download-AdoptOpenJdk([string] $url, [string] $jdkPath) {
	try {
		$ErrorActionPreference = "Stop";
		Invoke-WebRequest $url -OutFile $jdkPath
		Trace-Log "Download gateway successfully. Gateway loc: $jdkPath"
	}
	catch {
		Trace-Log "Fail to download gateway msi"
		Trace-Log $_.Exception.ToString()
...
function Install-AdoptOpenJdk([string] $jdkPath) {
	if ([string]::IsNullOrEmpty($jdkPath)) {
		Throw-Error "Path to AdoptOpenJdk is not specified"
	}

	if (!(Test-Path -Path $jdkPath)) {
		Throw-Error "Invalid AdoptOpenJdk path: $jdkPath"
	}
	
	Write-Host "Start AdoptOpenJdk installation"
	Start-Process -Wait -FilePath msiexec -ArgumentList /i, $jdkPath, "ADDLOCAL=FeatureMain,FeatureEnvironment,FeatureJarFileRunWith,FeatureJavaHome", /quiet, /norestart -PassThru -NoNewWindow 
	Start-Sleep -Seconds 30	

	Write-Host "Installation of AdoptOpenJdk is successful"
}
...
$url = "https://api.adoptopenjdk.net/v3/installer/latest/8/ga/windows/x64/jdk/hotspot/normal/adoptopenjdk?project=jdk"
$jdkPath = "$PWD\openjdk8.msi"
